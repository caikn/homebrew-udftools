...
{
	tag *t = desc;
	uint16_t crc_length = le16_to_cpu(t->descCRCLength);
	if (crc_length > length - sizeof(*t))
		return 0;
	if (compute_checksum(t) != t->tagChecksum)
		return 0;
	if (compute_crc(desc, sizeof(*t) + crc_length) != le16_to_cpu(t->descCRC))
		return 0;
	return 1;
}
...
{
	tag *t = desc;
	if (length > le16_to_cpu(t->descCRCLength) + sizeof(*t))
		length = le16_to_cpu(t->descCRCLength) + sizeof(*t);
	t->descCRC = cpu_to_le16(compute_crc(desc, length));
	t->tagChecksum = compute_checksum(t);
}
