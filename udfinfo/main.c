...
int main(int argc, char *argv[])
{
	...
	{
		uint32_t offset = 0;
		uint32_t npm = le32_to_cpu(lvd->numPartitionMaps);
		uint32_t mtl = le32_to_cpu(lvd->mapTableLength);
		struct metadataPartitionMap *mpm = NULL;
		uint32_t pi;

		for (pi = 0; pi < npm && offset < mtl; ++pi)
		{
			struct genericPartitionMap *gpm = (struct genericPartitionMap *)&lvd->partitionMaps[offset];
			if (gpm->partitionMapType == GP_PARTITION_MAP_TYPE_2)
			{
				struct udfPartitionMap2 *upm2 = (struct udfPartitionMap2 *)gpm;
				if (strncmp((char *)upm2->partIdent.ident, UDF_ID_METADATA, sizeof(upm2->partIdent.ident)) == 0)
				{
					mpm = (struct metadataPartitionMap *)gpm;
					break;
				}
			}
			offset += gpm->partitionMapLength;
		}
		...
		if (mpm)
		{
			...
			uint32_t mi;
			for (mi = 0; mi < disc.metadata_filemap_count[0]; ++mi)
				meta_size += (le32_to_cpu(disc.metadata_filemap[0][mi].extLength) & EXT_LENGTH_MASK) / disc.blocksize;

			printf("metadatapartition=yes\n");
			printf("metadatafileloc=%"PRIu32"\n", le32_to_cpu(mpm->metadataFileLoc));
			printf("metadatamirrorfileloc=%"PRIu32"\n", le32_to_cpu(mpm->metadataMirrorFileLoc));
			if (le32_to_cpu(mpm->metadataBitmapFileLoc) != 0xFFFFFFFF)
				printf("metadatabitmapfileloc=%"PRIu32"\n", le32_to_cpu(mpm->metadataBitmapFileLoc));
			else
				printf("metadatabitmapfileloc=none\n");
			printf("metadataallocunitsize=%"PRIu32"\n", le32_to_cpu(mpm->allocUnitSize));
			printf("metadataalignunitsize=%"PRIu16"\n", le16_to_cpu(mpm->alignUnitSize));
			printf("metadatasize=%"PRIu32"\n", meta_size);
			printf("metadataflags=%u\n", mpm->flags);
		}
		else
		{
			printf("metadatapartition=no\n");
		}
	}
